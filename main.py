import math
from tkinter import *


# ---------------------------- TIMER MECHANISM ------------------------------- #
def count_down(count):
    global time_loop
    global marks
    count_min = math.floor(count / 60)
    count_sec = count % 60
    if count_sec < 10:
        count_sec = f"0{count_sec}"

    canvas.itemconfig(timer_text, text=f"{count_min}:{count_sec}")
    if count > 0:
        time_loop = window.after(1000, count_down, count - 1)
    else:
        start_timer()
        for _ in range(math.floor(reps/2)):
            marks += "✔"
            label1.config(text=marks)


def start_timer():
    global reps
    work_sec = WORK_MIN * 60
    short_break_sec = SHORT_BREAK_MIN * 60
    long_break_sec = LONG_BREAK_MIN * 60

    reps += 1
    if reps < 9:
        if reps % 2 == 1:
            label.config(text="Work", fg=GREEN)
            count_down(work_sec)
        elif reps == 8:
            reps = 0
            label.config(text="Long Break", fg=RED)
            count_down(long_break_sec)
        else:
            label.config(text="Short Break", fg=PINK)
            count_down(short_break_sec)


def reset_timer():
    global marks
    global reps
    window.after_cancel(time_loop)
    canvas.itemconfig(timer_text, text="00:00")
    label.config(text="Timer", fg=GREEN)
    reps = 0
    marks = ""
    label1.config(text=marks)


# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
SKIN = "#ffdab9"
FONT = ("Amatic SC", 40, "bold")
WORK_MIN = 25
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 20
reps = 0
marks = ""
time_loop = None

# ---------------------------- TIMER RESET ------------------------------- #


# ---------------------------- COUNTDOWN MECHANISM ------------------------------- # 


# ---------------------------- UI SETUP ------------------------------- #
window = Tk()
window.title("Pomodoro Productivity Timer")
window.config(padx=100, pady=50, bg=SKIN)


# Start Button
start_button = Button(text="Start", command=start_timer)
start_button.grid(column=0, row=2)


# Image
canvas = Canvas(width=200, height=224, bg=SKIN, highlightthickness=0)
tomato_img = PhotoImage(file="tomato.png")
canvas.create_image(100, 112, image=tomato_img)
timer_text = canvas.create_text(100, 130, text="00:00", fill="white", font=(FONT, 30, "bold"))
canvas.grid(column=1, row=1)


# Label
label = Label(text="Timer", font=FONT, bg=SKIN, fg=GREEN, highlightthickness=0)
label.grid(column=1, row=0)


# Reset Button
reset_button = Button(text="Reset", command=reset_timer)
reset_button.grid(column=2, row=2)


# Checkmarks
label1 = Label(font=(FONT, 15, "bold"), bg=SKIN, fg=GREEN)
label1.grid(column=1, row=3)


window.mainloop()
